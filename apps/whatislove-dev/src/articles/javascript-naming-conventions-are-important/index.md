...
In JavaScript projects, there are numerous conventions, but the most valuable one to adhere to is maintaining a consistent coding style within a single project. Individuals involved in different projects know better than anyone that writing styles can vary significantly between projects.
...
Suppose you are working with a network request library like [axios](https://www.npmjs.com/package/axios), and the library's README recommends using the standard name `axios` for imports.
...
The name of a function/method should be a verb and correspond to the action it performs.
...
When working with classes, it is important to follow several conventions. Here is the list:
...
Used to describe values which are known _before the program's execution_ and that _should not change_ during its execution.
...
Also known as enumerations. In JavaScript, this _data structure_ is used to enumerate a set of fixed values.
...
Also known as a `dictionary` data structure. This data structure is used for _mapping_ one value to another.
...
As mentioned earlier, TypeScript has become an integral part of JavaScript development today. In general, _in most cases_, types and interfaces are interchangeable. However, since there are already enough discussions about which one to choose, in this article, we will focus specifically on naming. For types and interfaces, the following naming conventions exist:
