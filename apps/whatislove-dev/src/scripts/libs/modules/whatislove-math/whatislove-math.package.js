...
class WhatisloveMath {
	...
	/**
	 * @param {number} professionalLevel
	 * @param {ValuesOf<WhatisloveMath.BoostEntity>[]} boostEntities
	 * @returns {number}
	 * @throws {SyntaxError}
	 */
	static calculateProfessionalLevel(professionalLevel = 0, ...boostEntities) {
		let {
			BoostEntity,
		...
		let hasBoostEntities = boostEntities.length > EMPTY_LENGTH_COUNT

		if (!hasBoostEntities) {
			return professionalLevel - NOTHING_TODO_VALUE
		}
